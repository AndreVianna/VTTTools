...
public class BlobStorageHealthCheckTests {
    ...
    private const string TestConnectionString = "DefaultEndpointsProtocol=https;AccountName=teststorage;AccountKey=testkey;EndpointSuffix=core.windows.net";
    ...
    public void Constructor_ValidParameters_CreatesInstance() {
        ...
        // Act
        var healthCheck = new BlobStorageHealthCheck(_configuration, ConnectionStringName, ContainerName);
    ...
    public void Constructor_NullConnectionString_ThrowsArgumentException() {
        ...
        // Act & Assert
        var action = () => new BlobStorageHealthCheck(_configuration, ConnectionStringName, ContainerName);
    ...
    public void Constructor_NullContainerName_ThrowsArgumentNullException() {
        ...
        // Act & Assert
        var action = () => new BlobStorageHealthCheck(_configuration, ConnectionStringName, null!);
    ...
    public void Constructor_EmptyContainerName_DoesNotThrow() {
        ...
        // Act & Assert
        var action = () => new BlobStorageHealthCheck(_configuration, ConnectionStringName, "");
    ...
    public async Task CheckHealthAsync_ContainerDoesNotExist_ReturnsDegraded() {
        // Arrange
        var healthCheck = new MockableBlobStorageHealthCheck(ContainerName, containerExists: false);
        var context = new HealthCheckContext();
    ...
    public async Task CheckHealthAsync_ContainerExistsWithProperties_IncludesPropertiesInData() {
        ...
        var healthCheck = new MockableBlobStorageHealthCheck(ContainerName, 
    ...
    public async Task CheckHealthAsync_RequestFailedExceptionWithSpecificErrorCode_ReturnsAppropriateMessage(string errorCode, string expectedMessage) {
        ...
        var healthCheck = new MockableBlobStorageHealthCheck(ContainerName, existsException: requestException);
        var context = new HealthCheckContext();
    ...
    public async Task CheckHealthAsync_GeneralException_ReturnsUnhealthyWithGenericError() {
        ...
        var healthCheck = new MockableBlobStorageHealthCheck(ContainerName, existsException: exception);
        var context = new HealthCheckContext();
    ...
    public async Task CheckHealthAsync_CancellationTokenCancelled_ThrowsOperationCancelledException() {
        // Arrange
        var healthCheck = new MockableBlobStorageHealthCheck(ContainerName, simulateCancellation: true);
        var context = new HealthCheckContext();
    ...
    private class MockableBlobStorageHealthCheck : IHealthCheck {
        private readonly string _containerName;
        private readonly bool _containerExists;
        private readonly DateTimeOffset? _lastModified;
        private readonly PublicAccessType? _publicAccess;
        private readonly Exception? _existsException;
        private readonly Exception? _propertiesException;
        private readonly bool _simulateCancellation;
        private readonly TimeSpan _simulateDelay;

        public MockableBlobStorageHealthCheck(string containerName, bool containerExists = true, DateTimeOffset? lastModified = null, 
            PublicAccessType? publicAccess = null, Exception? existsException = null, Exception? propertiesException = null, 
            bool simulateCancellation = false, TimeSpan simulateDelay = default) {
            _containerName = containerName;
            _containerExists = containerExists;
            _lastModified = lastModified;
            _publicAccess = publicAccess;
            _existsException = existsException;
            _propertiesException = propertiesException;
