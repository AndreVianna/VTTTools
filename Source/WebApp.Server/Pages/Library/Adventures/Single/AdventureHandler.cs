...
namespace VttTools.WebApp.Server.Pages.Library.Adventures.Single;
...
public class AdventureHandler(AdventurePage page)
    ...
    public async Task<bool> LoadAdventureAsync(IAdventuresHttpClient adventures) {
        ...
        if (Page.Id == Guid.Empty)
            return false;
        var adventure = await _adventures.GetAdventureByIdAsync(Page.Id);
        if (adventure == null)
            return false;
    ...
    internal async Task DeleteAdventureAsync() {
        var deleted = await _adventures.DeleteAdventureAsync(Page.Id);
        if (deleted)
    ...
    private async Task UpdateAdventureAsync() {
        ...
        var result = await _adventures.UpdateAdventureAsync(Page.Id, request);
    ...
    private async Task CreateAdventureAsync() {
        ...
        var result = await _adventures.CreateAdventureAsync(request);
        if (!result.IsSuccessful) {
            Page.State.Errors = [.. result.Errors];
            await Page.StateHasChangedAsync();
        }
    ...
    internal async Task CreateSceneAsync() {
        var result = await _adventures.CreateSceneAsync(Page.Id);
        if (result.IsSuccessful) {
