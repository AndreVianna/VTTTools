    ...
    endpoints: (builder) => ({
        ...
        getEncounterAssets: builder.query<EncounterAsset[], string>({
            ...
            providesTags: (result, _error, encounterId) =>
                result
                    ? [
                        ...result.map(({ id }) => ({ type: 'EncounterAsset' as const, id })),
                        { type: 'EncounterAsset', id: `ENCOUNTER_${encounterId}` }
                    ]
                    : [{ type: 'EncounterAsset', id: `ENCOUNTER_${encounterId}` }]
        }),

        addEncounterAsset: builder.mutation<void, {
            encounterId: string;
        ...
        updateEncounterAsset: builder.mutation<void, {
            encounterId: string;
            assetNumber: number;
            position?: { x: number; y: number };
            size?: { width: number; height: number };
            ...
            query: ({ encounterId, assetNumber, position, size, rotation, name, tokenId, portraitId, notes, visible, locked }) => ({
        ...
        bulkAddEncounterAssets: builder.mutation<void, {
            encounterId: string;
            assets: Array<{
                assetId: string;
                position: { x: number; y: number };
        ...
        getEncounterWalls: builder.query<EncounterWall[], string>({
            ...
            providesTags: (result, _error, encounterId) => [
                ...(result?.map(({ index }) => ({ type: 'EncounterWall' as const, id: `${encounterId}-${index}` })) ?? []),
                { type: 'EncounterWall', id: `ENCOUNTER_${encounterId}` }
            ]
        }),

        addEncounterWall: builder.mutation<EncounterWall, {
            encounterId: string;
        ...
        updateEncounterWall: builder.mutation<void, {
            encounterId: string;
            wallIndex: number;
            name?: string | undefined;
            poles?: Pole[] | undefined;
        ...
        getEncounterRegions: builder.query<EncounterRegion[], string>({
            ...
            providesTags: (result, _error, encounterId) => [
                ...(result?.map(({ index }) => ({ type: 'EncounterRegion' as const, id: `${encounterId}-${index}` })) ?? []),
                { type: 'EncounterRegion', id: `ENCOUNTER_${encounterId}` }
            ]
        }),

        addEncounterRegion: builder.mutation<EncounterRegion, {
            encounterId: string;
            name: string;
            type: string;
            vertices: Point[];
        ...
        updateEncounterRegion: builder.mutation<void, {
            encounterId: string;
            regionIndex: number;
            name?: string;
            type?: string;
        ...
        getEncounterSources: builder.query<EncounterSource[], string>({
            ...
            providesTags: (result, _error, encounterId) => [
                ...(result?.map(({ index }) => ({ type: 'EncounterSource' as const, id: `${encounterId}-${index}` })) ?? []),
                { type: 'EncounterSource', id: `ENCOUNTER_${encounterId}` }
            ]
        }),

        addEncounterSource: builder.mutation<EncounterSource, {
            encounterId: string;
            name: string;
            type: string;
            position: Point;
        ...
        updateEncounterSource: builder.mutation<void, {
            encounterId: string;
            sourceIndex: number;
            name?: string;
            type?: string;
