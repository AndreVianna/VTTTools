...
export const useAssetManagement = ({
  ...
  const handleAssetPlaced = useCallback(
    (asset: PlacedAsset) => {
  ...
  const handleAssetMoved = useCallback(
    (
      ...
      if (moves.length === 1) {
        const move = moves[0];
        const { assetId, oldPosition, newPosition } = move as {
          assetId: string;
          oldPosition: { x: number; y: number };
          newPosition: { x: number; y: number };
        };
  ...
  const handleAssetRotated = useCallback(
    (
      updates: Array<{
        assetId: string;
        rotation: number;
  ...
  const handleRotationEnd = useCallback(async () => {
    ...
    const assetsToUpdate = placedAssets.filter((asset) => selectedAssetIds.includes(asset.id));
    ...
    const command = createTransformAssetCommand({
      ...
      onUpdate: (assetId, snapshot) => {
        setPlacedAssets((prev) => prev.map((a) => (a.id === assetId ? { ...a, ...snapshot } : a)));
      },
    });
      ...
      onBulkRemove: async (_assetIds) => {
        if (encounterId && isOnline && encounter) {
          try {
            ...
            await bulkDeleteEncounterAssets({
              encounterId,
