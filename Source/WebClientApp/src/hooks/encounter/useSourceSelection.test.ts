...
describe('useSourceSelection', () => {
    const createMockProps = () => ({
        ...
        setSelectedLightSourceIndex: vi.fn<(index: number | null) => void>(),
        setSelectedSoundSourceIndex: vi.fn<(index: number | null) => void>(),
        execute: vi.fn<() => Promise<void>>().mockResolvedValue(undefined),
        refetch: vi.fn<() => void>(),
        addLight: vi.fn<() => { unwrap: () => Promise<{ index: number }> }>(() => ({ unwrap: vi.fn<() => Promise<{ index: number }>>().mockResolvedValue({ index: 2 }) })),
        deleteLight: vi.fn<() => { unwrap: () => Promise<void> }>(() => ({ unwrap: vi.fn<() => Promise<void>>().mockResolvedValue(undefined) })),
        updateLight: vi.fn<() => { unwrap: () => Promise<void> }>(() => ({ unwrap: vi.fn<() => Promise<void>>().mockResolvedValue(undefined) })),
        addSound: vi.fn<() => { unwrap: () => Promise<{ index: number }> }>(() => ({ unwrap: vi.fn<() => Promise<{ index: number }>>().mockResolvedValue({ index: 1 }) })),
        deleteSound: vi.fn<() => { unwrap: () => Promise<void> }>(() => ({ unwrap: vi.fn<() => Promise<void>>().mockResolvedValue(undefined) })),
        updateSound: vi.fn<() => { unwrap: () => Promise<void> }>(() => ({ unwrap: vi.fn<() => Promise<void>>().mockResolvedValue(undefined) })),
    });
