...
export function AdventureListView() {
  ...
  return (
    <Box id='adventure-list-container'>
      ...
      {isLoading && (
        <Typography
          id='message-adventure-list-loading'
          variant='body1'
      ...
      {!isLoading && adventures.length === 0 && searchInput === '' && (
        <Box id='adventure-list-empty-state' sx={{ textAlign: 'center', py: 8 }}>
          ...
          <Button variant='contained' startIcon={<AddIcon />} onClick={handleCreateAdventure}>
            Create Adventure
          </Button>
        </Box>
      )}
      ...
      {hasMore && !isFetching && <div ref={sentinelRef} style={{ height: 1 }} />}

      {!hasMore && adventures.length > 0 && (
        <Typography sx={{ textAlign: 'center', py: 4, color: 'text.secondary' }}>All adventures loaded</Typography>
      )}

      <Dialog open={deleteDialogOpen} onClose={cancelDelete} aria-labelledby='dialog-title-delete-adventure'>
        <DialogTitle id='dialog-title-delete-adventure'>Delete Adventure</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete this adventure? This will also delete all encounters within it. This action
            cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
