...
export function WorldCard({ world, onOpen, onDuplicate, onDelete }: WorldCardProps) {
    ...
    const badges = (
        <>
            {world.isPublished && <PublishedBadge />}
        </>
    );
    ...
    const backgroundUrl = world.background
        ? `${apiEndpoints.media}/${world.background.id}`
        : WORLD_DEFAULT_BACKGROUND;
