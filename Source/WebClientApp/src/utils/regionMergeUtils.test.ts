...
describe('findMergeableRegions', () => {
  const createRegion = (type: string, vertices: Point[], value?: number, label?: string): EncounterRegion => ({
    encounterId: 'encounter1',
    index: 0,
    name: 'Region',
    type,
...
describe('mergePolygons', () => {
  ...
  it('should apply grid snapping when gridConfig provided with intersections', async () => {
    ...
    vi.mocked(lineLineIntersection.lineLineIntersection).mockReturnValue({
      x: 10,
      y: 5,
    });
