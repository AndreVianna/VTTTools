...
function segmentIntersection(
    ...
    const epsilon = tolerance / Math.max(
    ...
    if (t1 >= -epsilon && t1 <= 1 + epsilon && t2 >= -epsilon && t2 <= 1 + epsilon) {
        if ((Math.abs(t1) < epsilon && Math.abs(t2 - 1) < epsilon) ||
...
export function buildPlanarSubdivision(
    ...
    for (let i = 0; i < poles.length - 1; i++) {
        ...
        for (let j = 0; j < points.length - 1; j++) {
            ...
            const existingEdge1 = v1.edges.find(e =>
                e.to === v2.id && Math.abs(e.angle - angle12) < 1e-6
            );
            const existingEdge2 = v2.edges.find(e =>
                e.to === v1.id && Math.abs(e.angle - angle21) < 1e-6
            );
...
export function decomposeSelfIntersectingPath(
    poles: Point[],
    tolerance: number = 5
): DecomposedPath {
    if (poles.length < 2) {
    ...
    if (closedSegments.length > 0 && intersections.length > 0) {
        try {
            ...
            if (faces.length > 0) {
                closedWalls = faces
                    .filter(face => face.poles.length >= 3)
                    .map(face => face.poles);
            }
        } catch (error) {
