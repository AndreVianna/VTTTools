...
const createMockWall = (overrides?: Partial<EncounterWall>): StageWall => ({
    index: 0,
    name: 'Wall',
    segments: [],
    ...overrides,
...
const createMockRegion = (overrides?: Partial<EncounterRegion>): StageRegion => ({
    index: 0,
    name: 'Region',
...
describe('encounterStateUtils', () => {
    describe('addWallOptimistic', () => {
        it('should add wall to empty walls array', () => {
            ...
            expect(result.stage.walls).toHaveLength(1);
            expect(result.stage.walls[0]).toEqual(wall);
        });

        it('should add wall to existing walls', () => {
            const existingWall = createMockWall({ index: 1 });
            const encounter = createMockEncounter({
                stage: createMockStage({ walls: [existingWall] }),
            });
            const newWall = createMockWall({ index: 2 });

            const result = addWallOptimistic(encounter, newWall);

            expect(result.stage.walls).toHaveLength(2);
            expect(result.stage.walls[1]).toEqual(newWall);
        });
        ...
        it('should filter out temporary walls (index === -1) before adding', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ walls: [permanentWall, tempWall] }),
            });
            ...
            expect(result.stage.walls).toHaveLength(2);
            expect(result.stage.walls.find((w: StageWall) => w.name === 'Temp')).toBeUndefined();
            expect(result.stage.walls.find((w: StageWall) => w.name === 'Permanent')).toBeDefined();
            expect(result.stage.walls.find((w: StageWall) => w.name === 'New')).toBeDefined();
        });
        ...
        it('should preserve other encounter properties', () => {
            ...
            expect(result.stage.grid).toEqual(encounter.stage.grid);
        });
    });
    ...
    describe('updateWallOptimistic', () => {
        it('should update wall by index', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ walls: [wall] }),
            });

            const result = updateWallOptimistic(encounter, 1, { name: 'Updated' });

            expect(result.stage.walls[0]?.name).toBe('Updated');
        });
        ...
        it('should update only the specified wall', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ walls: [wall1, wall2] }),
            });

            const result = updateWallOptimistic(encounter, 2, {
                name: 'Updated Wall 2',
            });

            expect(result.stage.walls[0]?.name).toBe('Wall 1');
            expect(result.stage.walls[1]?.name).toBe('Updated Wall 2');
        });
    ...
    describe('removeWallOptimistic', () => {
        it('should remove wall by index', () => {
            ...
            expect(result.stage.walls).toHaveLength(0);
        });
        ...
        it('should keep other walls when removing one', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ walls: [wall1, wall2, wall3] }),
            });

            const result = removeWallOptimistic(encounter, 2);

            expect(result.stage.walls).toHaveLength(2);
            expect(result.stage.walls.find((w: StageWall) => w.name === 'Wall 1')).toBeDefined();
            expect(result.stage.walls.find((w: StageWall) => w.name === 'Wall 2')).toBeUndefined();
            expect(result.stage.walls.find((w: StageWall) => w.name === 'Wall 3')).toBeDefined();
        });
    ...
    describe('syncWallIndices', () => {
        ...
        it('should handle multiple temp walls', () => {
            ...
            const permanentWall = createMockWall({ index: 1, name: 'Permanent' });
            const encounter = createMockEncounter({
                stage: createMockStage({ walls: [tempWall1, permanentWall, tempWall2] }),
            });
            ...
            expect(result.stage.walls[0]?.index).toBe(10);
            expect(result.stage.walls[1]?.index).toBe(1);
            expect(result.stage.walls[2]?.index).toBe(11);
        });
    ...
    describe('removeTempRegions', () => {
        it('should remove regions with index === -1', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [tempRegion, permanentRegion] }),
            });

            const result = removeTempRegions(encounter);

            expect(result.stage.regions).toHaveLength(1);
            expect(result.stage.regions[0]?.name).toBe('Permanent');
        });
        ...
        it('should preserve regions with valid indices', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [region1, region2] }),
            });

            const result = removeTempRegions(encounter);

            expect(result.stage.regions).toHaveLength(2);
            expect(result.stage.regions).toEqual([region1, region2]);
        });

        it('should return unchanged encounter when no temp regions exist', () => {
            const region = createMockRegion({ index: 1 });
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [region] }),
            });

            const result = removeTempRegions(encounter);

            expect(result.stage.regions).toHaveLength(1);
            expect(result.stage.regions[0]).toEqual(region);
        });
        ...
        it('should handle encounter with all temp regions', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [tempRegion1, tempRegion2] }),
            });

            const result = removeTempRegions(encounter);

            expect(result.stage.regions).toHaveLength(0);
        });
        ...
        it('should preserve encounter metadata', () => {
            ...
            const encounter = createMockEncounter({
                ...
                stage: createMockStage({ regions: [tempRegion] }),
            });
        ...
        it('should handle mixed temp and permanent regions', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [temp1, perm1, temp2, perm2] }),
            });

            const result = removeTempRegions(encounter);

            expect(result.stage.regions).toHaveLength(2);
            expect(result.stage.regions[0]?.name).toBe('Permanent 1');
            expect(result.stage.regions[1]?.name).toBe('Permanent 2');
        });
    });
    ...
    describe('addRegionOptimistic', () => {
        ...
        it('should add region and filter temp regions', () => {
            ...
            const permanentRegion = createMockRegion({ index: 1, name: 'Permanent' });
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [permanentRegion, tempRegion] }),
            });
            ...
            expect(result.stage.regions).toHaveLength(2);
            expect(result.stage.regions.find((r: StageRegion) => r.name === 'Temp')).toBeUndefined();
            expect(result.stage.regions.find((r: StageRegion) => r.name === 'Permanent')).toBeDefined();
            expect(result.stage.regions.find((r: StageRegion) => r.name === 'New')).toBeDefined();
        });
    ...
    describe('updateRegionOptimistic', () => {
        it('should update region by index', () => {
            ...
            expect(result.stage.regions[0]?.name).toBe('Updated');
        });
        ...
        it('should update only the specified region', () => {
            ...
            const result = updateRegionOptimistic(encounter, 2, {
                name: 'Updated Region 2',
            });

            expect(result.stage.regions[0]?.name).toBe('Region 1');
            expect(result.stage.regions[1]?.name).toBe('Updated Region 2');
        });
        ...
        it('should partially update region properties', () => {
            ...
            const result = updateRegionOptimistic(encounter, 1, {
                value: 5,
            });

            expect(result.stage.regions[0]?.name).toBe('Original');
            expect(result.stage.regions[0]?.type).toBe('difficult-terrain');
            expect(result.stage.regions[0]?.value).toBe(5);
        });
    });
    ...
    describe('removeRegionOptimistic', () => {
        ...
        it('should keep other regions when removing one', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [region1, region2, region3] }),
            });

            const result = removeRegionOptimistic(encounter, 2);

            expect(result.stage.regions).toHaveLength(2);
            expect(result.stage.regions.find((r: StageRegion) => r.name === 'Region 1')).toBeDefined();
            expect(result.stage.regions.find((r: StageRegion) => r.name === 'Region 2')).toBeUndefined();
            expect(result.stage.regions.find((r: StageRegion) => r.name === 'Region 3')).toBeDefined();
        });
    ...
    describe('syncRegionIndices', () => {
        it('should update temporary region indices to real indices', () => {
            ...
            expect(result.stage.regions[0]?.index).toBe(5);
        });

        it('should not update regions with positive indices', () => {
            const permanentRegion = createMockRegion({ index: 1 });
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [permanentRegion] }),
            });
            const tempToRealMap = new Map([[-1, 5]]);

            const result = syncRegionIndices(encounter, tempToRealMap);

            expect(result.stage.regions[0]?.index).toBe(1);
        });
        ...
        it('should handle multiple temp regions', () => {
            ...
            const permanentRegion = createMockRegion({ index: 1, name: 'Permanent' });
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [tempRegion1, permanentRegion, tempRegion2] }),
            });
            const tempToRealMap = new Map([
                [-1, 10],
                [-2, 11],
            ...
            expect(result.stage.regions[0]?.index).toBe(10);
            expect(result.stage.regions[1]?.index).toBe(1);
            expect(result.stage.regions[2]?.index).toBe(11);
        });
    ...
    describe('filterEncounterForMergeDetection', () => {
        ...
        it('should filter temp regions and exclude specific region', () => {
            ...
            const region2 = createMockRegion({ index: 2, name: 'Region 2' });
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [tempRegion, region1, region2] }),
            });

            const result = filterEncounterForMergeDetection(encounter, {
                excludeRegionIndex: 1,
            });

            expect(result).not.toBeNull();
            expect(result?.stage.regions).toHaveLength(1);
            expect(result?.stage.regions[0]?.name).toBe('Region 2');
        });
        ...
        it('should preserve all other region data', () => {
            ...
            expect(result).not.toBeNull();
            expect(result?.stage.regions[0]).toEqual(region);
            expect(result?.stage.regions[0]?.vertices).toEqual(region.vertices);
            expect(result?.stage.regions[0]?.value).toBe(5);
        });
        ...
        it('should handle excludeRegionIndex with non-existent index', () => {
            const region1 = createMockRegion({ index: 1, name: 'Region 1' });
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [region1] }),
            });

            const result = filterEncounterForMergeDetection(encounter, {
                excludeRegionIndex: 99,
            });
        ...
        it('should handle mixed temp, excluded, and valid regions', () => {
            ...
            const encounter = createMockEncounter({
                stage: createMockStage({ regions: [temp1, perm1, excluded, temp2, perm2] }),
            });
