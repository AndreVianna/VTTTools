...
export function detectSplitPoints(params: {
    ...
    const edgeIntersections = detectEdgeOnEdgeIntersection(
        newWallPoles,
        existingWalls,
        gridConfig
    );
    ...
    const poleCollisions = detectPoleOnEdgeCollision(
        newWallPoles,
        existingWalls,
        gridConfig,
        tolerance
        ...
        if (!isNearExistingPole) {
            const isDuplicate = splits.some(
                existing =>
                    ...
                    calculateDistance(existing.splitPosition, collision.projectionPoint) <=
                        tolerance
            );
...
export function calculateDistanceAlongEdge(
    point: Point,
    edgeStart: Point,
    edgeEnd: Point
): number {
