import type { Asset, ObjectAsset, EncounterAsset, PlacedAsset, EncounterWall, PlacedWall, EncounterRegion, PlacedRegion, EncounterSource, PlacedSource } from '@/types/domain';
...
export function hydratePlacedRegions(
    encounterRegions: EncounterRegion[],
    encounterId: string
): PlacedRegion[] {
    return encounterRegions.map((region) => {
        const domId = getDomIdByIndex(encounterId, 'regions', region.index) ||
...
export function hydratePlacedSources(
    encounterSources: EncounterSource[],
    encounterId: string
): PlacedSource[] {
    return encounterSources.map((source) => {
        const domId = getDomIdByIndex(encounterId, 'sources', source.index) ||
