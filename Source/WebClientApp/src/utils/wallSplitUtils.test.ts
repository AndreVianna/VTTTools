...
describe('sortSplitPoints', () => {
    ...
    it('should sort splits by edge index', () => {
        ...
        const splits: SplitPoint[] = [
            { wallIndex: 0, edgeIndex: 2, splitPosition: { x: 50, y: 100 }, splitType: 'intersection' },
            { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 50, y: 0 }, splitType: 'intersection' },
            { wallIndex: 0, edgeIndex: 1, splitPosition: { x: 100, y: 50 }, splitType: 'intersection' }
        ];
    ...
    it('should sort multiple splits on same edge by distance along edge', () => {
        ...
        const splits: SplitPoint[] = [
            { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 75, y: 0 }, splitType: 'intersection' },
            { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 25, y: 0 }, splitType: 'pole-on-edge' }
        ];
    ...
    it('should handle mixed scenario with multiple edges and multiple splits per edge', () => {
        ...
        const splits: SplitPoint[] = [
            { wallIndex: 0, edgeIndex: 1, splitPosition: { x: 100, y: 75 }, splitType: 'intersection' },
            { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 75, y: 0 }, splitType: 'intersection' },
            { wallIndex: 0, edgeIndex: 1, splitPosition: { x: 100, y: 25 }, splitType: 'pole-on-edge' },
            { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 25, y: 0 }, splitType: 'pole-on-edge' }
        ];
...
describe('detectSplitPoints', () => {
    ...
    describe('edge cases', () => {
        ...
        it('should filter out split point very close to existing pole', () => {
            ...
            const result = detectSplitPoints({ newWallPoles, existingWalls, tolerance: 5 });
        ...
        it('should use custom tolerance parameter', () => {
            ...
            const resultCustom = detectSplitPoints({ newWallPoles, existingWalls, tolerance: 10 });
...
describe('splitWallAtPoints', () => {
    ...
    describe('height interpolation', () => {
        ...
        it('should interpolate height correctly for split at quarter point of sloped edge', () => {
            ...
            const splits: SplitPoint[] = [
                { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 25, y: 0 }, splitType: 'intersection' }
            ];
        ...
        it('should interpolate height correctly on vertical edge', () => {
            ...
            const splits: SplitPoint[] = [
                { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 0, y: 50 }, splitType: 'intersection' }
            ];
        ...
        it('should interpolate height correctly on diagonal edge', () => {
            const wall = createWall([
                createPole(0, 0, 0),
                createPole(100, 100, 10)
            ], 0);
            const splits: SplitPoint[] = [
                { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 50, y: 50 }, splitType: 'intersection' }
            ];
    ...
    describe('segment naming and properties', () => {
        it('should name segments with sequential numbers', () => {
            ...
            const splits: SplitPoint[] = [
                { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 33, y: 0 }, splitType: 'intersection' },
                { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 66, y: 0 }, splitType: 'intersection' }
            ];
    ...
    describe('edge cases', () => {
        ...
        it('should return original wall when no splits for this wallIndex', () => {
            ...
            const splits: SplitPoint[] = [
                { wallIndex: 1, edgeIndex: 0, splitPosition: { x: 50, y: 0 }, splitType: 'intersection' }
            ];
        ...
        it('should not split last edge of open wall', () => {
            ...
            const splits: SplitPoint[] = [
                { wallIndex: 0, edgeIndex: 2, splitPosition: { x: 50, y: 50 }, splitType: 'intersection' }
            ];
...
describe('complex integration', () => {
    ...
    it('should handle multiple splits creating 3+ segments', () => {
        ...
        const splits: SplitPoint[] = [
            { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 25, y: 0 }, splitType: 'pole-on-edge' },
            { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 50, y: 0 }, splitType: 'intersection' },
            { wallIndex: 0, edgeIndex: 0, splitPosition: { x: 75, y: 0 }, splitType: 'pole-on-edge' }
        ];
    ...
    it('should handle closed wall with splits on wrapping edge', () => {
        ...
        const splits: SplitPoint[] = [
            { wallIndex: 0, edgeIndex: 3, splitPosition: { x: 0, y: 50 }, splitType: 'intersection' }
        ];
