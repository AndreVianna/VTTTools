...
describe('encounterEntityMapping', () => {
    ...
    describe('clearEncounterMappings', () => {
        ...
        it('should not affect other encounters', () => {
            ...
            expect(getAllMappings('encounter-2', testEntityType)).toEqual({ 'dom-2': 1 });
        });
    ...
    describe('input validation', () => {
        describe('encounterId validation', () => {
            it('should throw error for empty encounterId', () => {
                expect(() => setEntityMapping('', testEntityType, testDomId, testIndex))
                    .toThrow('Invalid encounterId: must be non-empty string');
            });
            ...
            it('should throw error for encounterId with illegal characters', () => {
                expect(() => setEntityMapping('encounter<script>', testEntityType, testDomId, testIndex))
                    .toThrow('Invalid encounterId: contains illegal characters');

                expect(() => setEntityMapping('encounter id', testEntityType, testDomId, testIndex))
                    .toThrow('Invalid encounterId: contains illegal characters');
            ...
            it('should accept valid encounterId with alphanumeric, hyphens, and underscores', () => {
                expect(() => setEntityMapping('valid-encounter_123', testEntityType, testDomId, testIndex))
                    .not.toThrow();
            });
        });
        ...
        describe('domId validation', () => {
            it('should throw error for empty domId', () => {
                expect(() => setEntityMapping(testEncounterId, testEntityType, '', testIndex))
                    .toThrow('Invalid domId: must be non-empty string');
            });

            it('should throw error for invalid domId type', () => {
                expect(() => setEntityMapping(testEncounterId, testEntityType, undefined as any, testIndex))
                    .toThrow('Invalid domId: must be non-empty string');
            });

            it('should throw error for domId with illegal characters', () => {
                expect(() => setEntityMapping(testEncounterId, testEntityType, 'dom<img src=x>', testIndex))
                    .toThrow('Invalid domId: contains illegal characters');

                expect(() => setEntityMapping(testEncounterId, testEntityType, 'dom id', testIndex))
                    .toThrow('Invalid domId: contains illegal characters');
            });

            it('should accept valid domId with alphanumeric, hyphens, and underscores', () => {
                expect(() => setEntityMapping(testEncounterId, testEntityType, 'valid-dom_456', testIndex))
                    .not.toThrow();
            });
        });
        ...
        describe('validation in all functions', () => {
            it('should validate encounterId in getIndexByDomId', () => {
                expect(() => getIndexByDomId('invalid encounter', testEntityType, testDomId))
                    .toThrow('Invalid encounterId: contains illegal characters');
            });

            it('should validate domId in getIndexByDomId', () => {
                expect(() => getIndexByDomId(testEncounterId, testEntityType, 'invalid dom'))
                    .toThrow('Invalid domId: contains illegal characters');
            });
            ...
            it('should validate encounterId in removeEntityMapping', () => {
                expect(() => removeEntityMapping('invalid@encounter', testEntityType, testDomId))
                    .toThrow('Invalid encounterId: contains illegal characters');
            });

            it('should validate domId in removeEntityMapping', () => {
                expect(() => removeEntityMapping(testEncounterId, testEntityType, 'invalid@dom'))
                    .toThrow('Invalid domId: contains illegal characters');
            });
            ...
            it('should validate encounterId in clearEncounterMappings', () => {
                expect(() => clearEncounterMappings('invalid encounter'))
                    .toThrow('Invalid encounterId: contains illegal characters');
            });
        });
    });
    ...
    describe('corrupted localStorage data', () => {
        ...
        it('should return empty object for data with wrong structure', () => {
            localStorage.setItem('encounter-mappings', JSON.stringify({
                'encounter-1': {
    ...
    describe('localStorage persistence', () => {
        ...
        it('should handle localStorage errors gracefully', () => {
            ...
            expect(() => setEntityMapping(testEncounterId, testEntityType, testDomId, testIndex))
                .not.toThrow();
