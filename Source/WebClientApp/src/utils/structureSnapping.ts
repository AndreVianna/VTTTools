...
export function getSnapTargets(
    ...
    targets.push(
        ...
        { point: { x: pixelX + cellSize.width, y: pixelY + cellSize.height }, type: 'corner' }
    );

    if (mode === SnapMode.HalfSnap || mode === SnapMode.QuarterSnap) {
        targets.push(
            { point: { x: pixelX + cellSize.width / 2, y: pixelY }, type: 'edge' },
            { point: { x: pixelX + cellSize.width, y: pixelY + cellSize.height / 2 }, type: 'edge' },
            { point: { x: pixelX + cellSize.width / 2, y: pixelY + cellSize.height }, type: 'edge' },
            { point: { x: pixelX, y: pixelY + cellSize.height / 2 }, type: 'edge' },
            { point: { x: pixelX + cellSize.width / 2, y: pixelY + cellSize.height / 2 }, type: 'center' }
        );
    }
...
export function snapToNearest(
    ...
    for (const target of targets) {
        const distance = Math.sqrt(
            Math.pow(target.point.x - mousePos.x, 2) +
            Math.pow(target.point.y - mousePos.y, 2)
        );
