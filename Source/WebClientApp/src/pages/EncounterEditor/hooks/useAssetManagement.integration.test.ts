...
describe('useAssetManagement - Integration Tests for Undo/Redo with localStorage Mapping', () => {
            ...
            const command = {
                ...
                undo: vi.fn(async () => {
                    ...
                    if (retrievedIndex !== undefined) {
                        await mockRemoveEncounterAsset({ encounterId: testEncounterId, assetNumber: retrievedIndex }).unwrap();
    ...
    describe('Multiple Asset Operations', () => {
        it('should handle multiple asset placements with separate mappings', async () => {
            ...
            await act(async () => {
                ...
                if (retrievedIndex1 !== undefined) {
                    await mockRemoveEncounterAsset({ encounterId: testEncounterId, assetNumber: retrievedIndex1 }).unwrap();
                }

                const retrievedIndex2 = getIndexByDomId(testEncounterId, 'assets', 'asset-temp-2');
                if (retrievedIndex2 !== undefined) {
                    await mockRemoveEncounterAsset({ encounterId: testEncounterId, assetNumber: retrievedIndex2 }).unwrap();
                }
            });
        ...
        it('should undo multiple asset placements in reverse order', async () => {
            ...
            await act(async () => {
                ...
                if (index2 !== undefined) {
                    await mockRemoveEncounterAsset({ encounterId: testEncounterId, assetNumber: index2 }).unwrap();
            ...
            await act(async () => {
                ...
                if (index1 !== undefined) {
                    await mockRemoveEncounterAsset({ encounterId: testEncounterId, assetNumber: index1 }).unwrap();
    ...
    describe('localStorage Persistence', () => {
        ...
        it('should handle undo after page reload simulation', async () => {
            ...
            await act(async () => {
                await mockRemoveEncounterAsset({ encounterId: testEncounterId, assetNumber: backendIndex }).unwrap();
                removeEntityMapping(testEncounterId, 'assets', tempDomId);
            });
