...
import * as sceneStateUtils from '@/utils/sceneStateUtils';
import * as commands from '@/utils/commands';
import * as regionCommands from '@/utils/commands/regionCommands';

vi.mock('@/utils/sceneStateUtils');
vi.mock('@/utils/commands');
vi.mock('@/utils/commands/regionCommands');

const mockUpdateRegionOptimistic = vi.mocked(sceneStateUtils.updateRegionOptimistic);
const mockRemoveRegionOptimistic = vi.mocked(sceneStateUtils.removeRegionOptimistic);
const mockRemoveTempRegions = vi.mocked(sceneStateUtils.removeTempRegions);
...
describe('useMergeRegions', () => {
    ...
    let testScene: Scene;
    let targetRegion: SceneRegion;
    let regionToDelete: SceneRegion;

    beforeEach(() => {
        vi.clearAllMocks();

        mockAddSceneRegion = vi.fn();
        mockUpdateSceneRegion = vi.fn();
        mockRemoveSceneRegion = vi.fn();
        mockSetScene = vi.fn();
        mockSetErrorMessage = vi.fn();
        mockRecordAction = vi.fn();
        mockRefetch = vi.fn().mockResolvedValue({ data: {} as Scene });
        ...
        mockUpdateRegionOptimistic.mockImplementation((scene, regionIndex, changes) => ({
            ...scene,
            regions: scene.regions.map(r =>
                r.index === regionIndex ? { ...r, ...changes } : r
            )
        }));

        mockRemoveRegionOptimistic.mockImplementation((scene, regionIndex) => ({
            ...scene,
            regions: scene.regions.filter(r => r.index !== regionIndex)
        }));

        mockRemoveTempRegions.mockImplementation(scene => ({
            ...scene,
    ...
    describe('executeMerge - Successful Merges', () => {
        it('should merge with single target region successfully', async () => {
            ...
            mockUpdateSceneRegion.mockReturnValue({ unwrap: mockUnwrap });
            mockRemoveSceneRegion.mockReturnValue({ unwrap: mockUnwrap });
            ...
            expect(mockUpdateRegionOptimistic).toHaveBeenCalledWith(
                testScene,
                1,
                { vertices: mergedVertices }
            );
            expect(mockRemoveRegionOptimistic).toHaveBeenCalledWith(
                expect.any(Object),
                2
            );
        ...
        it('should merge with multiple regions to delete', async () => {
            ...
            vi.spyOn(regionCommands, 'DeleteRegionCommand').mockImplementation(() => {
                deleteCallCount++;
                return (deleteCallCount === 1 ? mockDeleteCommand1 : mockDeleteCommand2) as unknown as regionCommands.DeleteRegionCommand;
            });
    ...
    describe('executeMerge - Error Handling', () => {
        it('should return early when sceneId is undefined', async () => {
            const { result } = renderHook(() =>
                useMergeRegions({
                    sceneId: undefined,
                    scene: testScene,
        ...
        it('should return early when scene is null', async () => {
            const { result } = renderHook(() =>
                useMergeRegions({
                    sceneId: 'scene-1',
                    scene: null,
        ...
        it('should handle EditRegionCommand execution failure', async () => {
            ...
            const mergedVertices: Point[] = [{ x: 0, y: 0 }, { x: 100, y: 0 }, { x: 50, y: 100 }];
            const onSuccess = vi.fn();
            const onError = vi.fn();
    ...
    describe('executeMerge - Command Creation', () => {
        it('should create EditRegionCommand with correct parameters', async () => {
            ...
            const editCommandSpy = vi.spyOn(regionCommands, 'EditRegionCommand').mockImplementation(
                () => mockEditCommand as unknown as regionCommands.EditRegionCommand
            );
        ...
        it('should create DeleteRegionCommand for each region to delete', async () => {
            ...
            const mergedVertices: Point[] = [{ x: 0, y: 0 }, { x: 150, y: 0 }];
            const onSuccess = vi.fn();
            const onError = vi.fn();
            ...
            const deleteCommandSpy = vi.spyOn(regionCommands, 'DeleteRegionCommand').mockImplementation(
                () => ({ execute: vi.fn().mockResolvedValue(undefined) } as unknown as regionCommands.DeleteRegionCommand)
            );
        ...
        it('should create BatchCommand wrapping all commands', async () => {
            ...
            const mergedVertices: Point[] = [{ x: 0, y: 0 }, { x: 100, y: 0 }];
            const onSuccess = vi.fn();
            const onError = vi.fn();
    ...
    describe('executeMerge - Optimistic State Updates', () => {
        it('should update scene state with merged region vertices', async () => {
            const { result } = renderHook(() =>
                useMergeRegions({
                    sceneId: 'scene-1',
                    scene: testScene,
            ...
            vi.spyOn(regionCommands, 'EditRegionCommand').mockImplementation(
                () => ({ execute: vi.fn().mockResolvedValue(undefined) } as unknown as regionCommands.EditRegionCommand)
            );
            vi.spyOn(regionCommands, 'DeleteRegionCommand').mockImplementation(
                () => ({ execute: vi.fn().mockResolvedValue(undefined) } as unknown as regionCommands.DeleteRegionCommand)
            );
        ...
        it('should remove deleted regions from scene state', async () => {
            const region3: SceneRegion = {
                index: 3,
                name: 'Region 3',
                vertices: [{ x: 100, y: 0 }, { x: 150, y: 0 }, { x: 125, y: 50 }],
            ...
            expect(mockRemoveRegionOptimistic).toHaveBeenNthCalledWith(
                1,
                expect.any(Object),
                2
            );
        ...
        it('should remove temporary regions from scene state', async () => {
            const { result } = renderHook(() =>
                useMergeRegions({
                    sceneId: 'scene-1',
                    scene: testScene,
        ...
        it('should call setScene with final optimistic state', async () => {
            const { result } = renderHook(() =>
                useMergeRegions({
                    sceneId: 'scene-1',
                    scene: testScene,
            ...
            mockRemoveTempRegions.mockReturnValue(finalScene);
            ...
            expect(mockSetScene).toHaveBeenCalledWith(finalScene);
        });
    });
    ...
    describe('executeMerge - Callback Invocations', () => {
        ...
        it('should call onError after failed merge', async () => {
            ...
            vi.spyOn(regionCommands, 'EditRegionCommand').mockImplementation(
                () => ({ execute: vi.fn().mockRejectedValue(new Error('Failed')) } as unknown as regionCommands.EditRegionCommand)
            );
