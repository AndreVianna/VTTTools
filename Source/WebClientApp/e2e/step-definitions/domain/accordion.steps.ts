...
Then(
  'the {string} accordion should show {string} badge',
  async function (this: CustomWorld, accordionName: string, badgeText: string) {
...
Then(
  'the {string} accordion should appear and be collapsed',
  async function (this: CustomWorld, accordionName: string) {
    await expect(this.page.locator(`button:has-text("${accordionName}")`)).toBeVisible();
    await verifyAccordionCollapsed(this.page, accordionName);
  },
);
