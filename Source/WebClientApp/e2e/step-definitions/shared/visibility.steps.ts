...
Then('I should see {int} cards total \\({int} {string} card + {int} asset cards\\)', async function (
    this: CustomWorld,
    totalCount: number,
    _addCount: number,
    _addType: string,
    ...
    await expect(this.page.locator('[data-testid="asset-card"], [data-testid="virtual-add-card"]')).toHaveCount(totalCount);
});
