...
When('I click {string} in the {string}', async function (this: CustomWorld, buttonText: string, containerName: string) {
  const container =
    containerName === 'confirmation dialog' ? '[role="dialog"]:last-of-type' : `[data-testid="${containerName}"]`;
...
When('I click the X button on the {word} image', async function (this: CustomWorld, position: string) {
  const index = position === 'first' ? 0 : position === 'second' ? 1 : parseInt(position, 10) - 1;
...
Then('the button should be visible and enabled', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify a specific button is visible and enabled (parameters expected or define button selector)',
  );
});
