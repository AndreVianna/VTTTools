...
Given('my encounter exists', async function (this: CustomWorld) {
    ...
    const dbEncounter = await this.db.queryTable('Encounters', { Id: encounterId });
...
Given('my encounter has stage with width {int} and height {int}', async function (this: CustomWorld, width: number, height: number) {
...
Given('my encounter has configured grid and {int} placed assets', async function (this: CustomWorld, assetCount: number) {
    const encounterId = this.currentAsset.id;
...
Given('a encounter exists owned by another user', async function (this: CustomWorld) {
    ...
    const ctx = this as any; ctx['otherUserEncounterId'] = '019639ea-c7de-7a01-8548-41edfccde888';
});
...
Given('no encounter exists with ID {string}', async function (this: CustomWorld, encounterId: string) {
    ...
    const encounter = await this.db.queryTable('Encounters', { Id: encounterId });
...
Then('all asset placements should remain intact', async function (this: CustomWorld) {
    ...
    const placements = await this.db.queryTable('EncounterAssets', { EncounterId: encounterId });
