...
Then('the PNG should be stored at blob path {string}', async function (this: CustomWorld, pathPattern: string) {
    ...
    const resources = await this.db.queryTable('Resources', { Id: lastResourceId });
...
Given('I assign image-{int} Token role \\(value: {int})', async function (this: CustomWorld, imageIndex: number, roleValue: number) {
...
Given('I assign image-{int} Display role \\(value: {int})', async function (this: CustomWorld, imageIndex: number, roleValue: number) {
...
Then('AssetResources table should contain {int} records:', async function (this: CustomWorld, _count: number, _dataTable: any) {
...
Then('each Resource should exist in Media.Resources table', async function (this: CustomWorld) {
    for (const resourceId of this.uploadedResourceIds) {
        const resources = await this.db.queryTable('Resources', { Id: resourceId });
        expect(resources.length).toBe(1);
    }
});
...
Given('I own asset with ID {string} having resources:', async function (this: CustomWorld, assetId: string, dataTable) {
    ...
    const rows = dataTable.hashes();
    for (const row of rows) {
        await this.db.insertAssetResource(assetId, row.ResourceId, parseInt(row.Role));
    }
...
When('I try to upload {string} \\({int}MB file)', async function (this: CustomWorld, filename: string, _sizeInMB: number) {
    await this.page.click('button:has-text("Upload")');
    await this.page.setInputFiles('input[type="file"]', `e2e/test-data/images/${filename}`);

    try {
        await this.page.waitForResponse(
            resp => resp.url().includes('/api/resources'),
            { timeout: 5000 }
        );
    } catch (error) {
...
Then('backend should reject with {int} Request Entity Too Large', async function (this: CustomWorld, statusCode: number) {
...
Then('the Manage panel background should be dark', async function (this: CustomWorld) {
    ...
    if (matches && matches[1] && matches[2] && matches[3]) {
        const [r, g, b] = [parseInt(matches[1]), parseInt(matches[2]), parseInt(matches[3])];
...
Then('text should be light for readability', async function (this: CustomWorld) {
    ...
    if (matches && matches[1] && matches[2] && matches[3]) {
        const [r, _g, _b] = [parseInt(matches[1]), parseInt(matches[2]), parseInt(matches[3])];
...
Then('images should be keyboard accessible', async function (this: CustomWorld) {
    ...
    const isFocused = await firstImage.evaluate(el => el === document.activeElement || el.contains(document.activeElement));
