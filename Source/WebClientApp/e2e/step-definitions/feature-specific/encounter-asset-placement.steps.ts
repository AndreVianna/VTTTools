...
Given('my encounter has configured stage', async function (this: CustomWorld) {
    ...
    const encounters = await this.db.queryTable('Encounters', { Id: this.currentEncounterId });
...
Given('my encounter has asset at position X={int}, Y={int}', async function (
    this: CustomWorld,
    x: number,
    y: number
) {
...
Given('my encounter has asset with width {int} and height {int}', async function (
    this: CustomWorld,
    width: number,
    height: number
) {
...
Given('my encounter has stage width {int} and height {int}', async function (
    this: CustomWorld,
    width: number,
    height: number
) {
...
Given('my encounter has {int} placed assets:', async function (
    this: CustomWorld,
    count: number,
    dataTable: any
) {
...
Given('my encounter has asset {string} at position X={int}, Y={int}', async function (
    this: CustomWorld,
    assetName: string,
    x: number,
    y: number
...
Given('no asset with ID {string} exists on my encounter', async function (
    this: CustomWorld,
    assetId: string
) {
    const assets = await this.db.queryTable('EncounterAssets', {
...
Given('no asset with ID {string} exists on the encounter', async function (
    this: CustomWorld,
    assetId: string
) {
    const assets = await this.db.queryTable('EncounterAssets', {
...
When('I attempt to place asset on encounter {string}', async function (
    this: CustomWorld,
    encounterId: string
) {
    try {
        ...
        this.lastApiResponse = await this.page.waitForResponse(
            (resp) => resp.url().includes('/api/encounters/'),
            { timeout: 5000 }
        ) as any;
    } catch (error) {
...
When('I attempt to move asset on encounter {string}', async function (
    this: CustomWorld,
    encounterId: string
) {
    try {
...
When('I attempt to remove asset from encounter {string}', async function (
    this: CustomWorld,
    encounterId: string
) {
    try {
...
Then('the position should be at origin', async function (this: CustomWorld) {
    ...
    const asset = await this.db.queryTable('EncounterAssets', { Id: this.currentAssetInstanceId });
...
Then('the encounter should now have {int} placed assets', async function (
    this: CustomWorld,
    count: number
) {
...
Then('the encounter should have {int} placed assets', async function (this: CustomWorld, count: number) {
    ...
    const actualCount = await getPlacedAssetCount(this.page, this.currentEncounterId!);
    expect(actualCount).toBe(count);
    const assets = await this.db.queryTable('EncounterAssets', { EncounterId: this.currentEncounterId });
    expect(assets.length).toBe(count);
});
...
Then('the stage configuration should remain unchanged', async function (this: CustomWorld) {
    const encounter = await this.db.queryTable('Encounters', { Id: this.currentEncounterId });
