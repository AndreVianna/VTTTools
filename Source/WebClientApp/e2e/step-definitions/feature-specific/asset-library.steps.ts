...
Given('{int} Object assets and {int} Creature assets exist in my library', async function (
    this: CustomWorld,
    objectCount: number,
...
Given('an asset exists with name {string} and description {string}', async function (
    this: CustomWorld,
    name: string,
    description: string
) {
...
Given('an Object asset exists with:', async function (this: CustomWorld, dataTable: any) {
    ...
    if (data.size) {
        ...
        builder.withSize(parseInt(widthStr), parseInt(heightStr));
    }
...
Given('a Creature asset exists with category {string}', async function (
    this: CustomWorld,
    category: string
) {
    const builder = this.assetBuilder()
...
Given('another user owns {int} public published Object assets', async function (
    this: CustomWorld,
    count: number
) {
...
Given('I am viewing page {int} with {string} shown', async function (
    this: CustomWorld,
    pageNumber: number,
    _pagesText: string
) {
...
Given('I have searched for {string} showing {int} results', async function (
    this: CustomWorld,
    searchQuery: string,
    _resultCount: number
) {
...
When('I immediately type {string} \\(within {int}ms\\)', async function (
    this: CustomWorld,
    character: string,
...
When('I rapidly type {string} \\(1 character every {int}ms\\)', async function (
    this: CustomWorld,
    text: string,
    delay: number
) {
...
Then('I should see {int} cards \\(1 {string} card + {int} asset cards\\)', async function (
    this: CustomWorld,
    totalCount: number,
    _addType: string,
    _assetCount: number
...
Then('I should see {int} cards \\(1 {string} + {int} {word} assets\\)', async function (
    this: CustomWorld,
    totalCount: number,
    _addCard: string,
    _assetCount: number,
...
Then('I should see {int} cards \\(1 {string} + {int} {word}\\)', async function (
    this: CustomWorld,
    totalCount: number,
    _addCard: string,
    _assetCount: number,
...
Then('I should see {int} cards on page {int} \\(1 {string} + {int} assets\\)', async function (
    this: CustomWorld,
    totalCount: number,
    _pageNumber: number,
    _addCard: string,
...
Then('the {string} card should say {string}', async function (
    this: CustomWorld,
    _cardType: string,
    expectedText: string
) {
...
Then('I should not see the {string} card during loading', async function (
    this: CustomWorld,
    _cardLabel: string
) {
...
Then('I should see {int} asset cards \\({word} Dragon, {word} Dragon\\)', async function (
    this: CustomWorld,
    count: number,
    _asset1: string,
    _asset2: string
...
Then('the API should be called once with search={string}', async function (
    ...
    const response = await this.page.waitForResponse((resp) =>
        resp.url().includes(`/api/assets`) && resp.url().includes(`search=${searchQuery}`)
    );
...
Then('the asset cards should show assets {int}-{int}', async function (
    this: CustomWorld,
    _start: number,
    _end: number
) {
...
Then('I should be on page {int}', async function (this: CustomWorld, pageNumber: number) {
    await expect(this.assetLibrary.pagination().locator(`button[aria-current="page"]:has-text("${pageNumber}")`)).toBeVisible();
});
...
Then('the asset card should show a {string} badge', async function (
    this: CustomWorld,
    badgeText: string
) {
    const card = this.assetLibrary.assetCards().first();
...
Then('the asset card should show the Token image {string}', async function (
    this: CustomWorld,
    imagePath: string
) {
    const card = this.assetLibrary.assetCards().first();
...
Then('the dialog kind should be locked to {string}', async function (
    this: CustomWorld,
    kind: string
) {
...
Then('the {word} tab should be selected in the dialog', async function (
    this: CustomWorld,
    tabName: string
) {
    await expect(this.page.locator(`button[role="tab"]:has-text("${tabName}")`)).toHaveAttribute('aria-selected', 'true');
...
Then('the dialog should show the {string} asset details', async function (
    this: CustomWorld,
    assetName: string
) {
...
Then('the virtual {string} card should display {string}', async function (
    this: CustomWorld,
    _cardType: string,
    expectedText: string
) {
...
Then('the initial page should load within {int}ms threshold', async function (
    this: CustomWorld,
    threshold: number
) {
...
Then('the page background should be dark', async function (this: CustomWorld) {
    ...
    expect(parseInt(r || '0')).toBeLessThan(50);
});
...
Then('the virtual {string} card should have dark styling', async function (
    this: CustomWorld,
    _cardType: string
) {
    const card = this.assetLibrary.virtualAddCard();
...
Then('the page background should be light', async function (this: CustomWorld) {
    ...
    expect(bg).toMatch(/rgb\((\d+), (\d+), (\d+)\)/);
    const [, r, _g, _b] = bg.match(/rgb\((\d+), (\d+), (\d+)\)/) || [];
    expect(parseInt(r || '0')).toBeGreaterThan(200);
});
