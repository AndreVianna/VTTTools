...
Then(
  'the image should have role {string} \\({int}) initially',
...
Then('the file input should reject other file types', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify file input with accept attribute rejects invalid file types (validate accept attribute is set to image formats)',
  );
});
...
Then('the resource should be stored as PNG', async function (this: CustomWorld) {
  ...
  const lastResourceId = this.uploadedResourceIds[this.uploadedResourceIds.length - 1];
  const resource = await this.db.queryTable('Resources', {
    Id: lastResourceId,
  });
...
When('I select a file to upload', async function (this: CustomWorld) {
  const fileInput = this.page.locator('input[type="file"]');
  await fileInput.setInputFiles('e2e/test-data/images/test.png', {
    noWaitAfter: true,
  });
});
...
When('upload completes', async function (this: CustomWorld) {
  await this.page.waitForResponse((resp) => resp.url().includes('/api/resources') && resp.status() === 201);
});
...
Given(
  'an image has role {string} \\({int})',
  async function (this: CustomWorld, _roleName: string, _roleValue: number) {
    throw new Error(
      'OBSOLETE: Role assignment via keyboard shortcuts no longer exists. Use token/portrait upload methods instead.',
    );
  },
);

Given('the image has role {string}', async function (this: CustomWorld, _roleName: string) {
  throw new Error(
    'OBSOLETE: Role assignment via keyboard shortcuts no longer exists. Use token/portrait upload methods instead.',
  );
});
...
Then(
  'the image role should change to {string} \\({int})',
  async function (this: CustomWorld, _roleName: string, roleValue: number) {
    const lastResourceId = this.uploadedResourceIds[this.uploadedResourceIds.length - 1];
    const image = this.page.locator(`[data-resource-id="${lastResourceId}"]`);
    await expect(image).toHaveAttribute('data-role', roleValue.toString(), {
...
Then(
  'role should toggle back to {string} \\({int})',
  async function (this: CustomWorld, _roleName: string, roleValue: number) {
    const lastResourceId = this.uploadedResourceIds[this.uploadedResourceIds.length - 1];
    const image = this.page.locator(`[data-resource-id="${lastResourceId}"]`);
    await expect(image).toHaveAttribute('data-role', roleValue.toString());
...
Then(
  'the role should remain {string} \\({int})',
  async function (this: CustomWorld, _roleName: string, roleValue: number) {
    const lastResourceId = this.uploadedResourceIds[this.uploadedResourceIds.length - 1];
    const image = this.page.locator(`[data-resource-id="${lastResourceId}"]`);
    await expect(image).toHaveAttribute('data-role', roleValue.toString());
...
Then('no role change should occur', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify clicking image without modifiers does not change role (verify data-role attribute remains unchanged)',
  );
});
...
Given('I am editing an asset with {int} existing images', async function (this: CustomWorld, count: number) {
  ...
  const asset = await this.assetBuilder().withName('Test Asset').withResources(count).build();
...
Then('should not show {string} or {string}', async function (this: CustomWorld, _img1: string, _img2: string) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify Token preview shows only first Token image, not other Token images (verify only one image appears in token-preview)',
  );
});
...
Given('no images have Display role', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to ensure no images in uploadedResourceIds have Display role (verify no images have data-role="2" or data-role="3")',
  );
});
...
Given(
  'I have {int} image with role {string} \\({int})',
  async function (this: CustomWorld, count: number, _roleName: string, roleValue: number) {
    expect(count).toBe(1);
    if (roleValue === 3) {
      const tokenId = await uploadToken(this.page, 'test-image.png', true);
      this.uploadedResourceIds.push(tokenId);
...
When('viewing on different screen sizes', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to simulate viewport changes and verify grid responsiveness (set viewport sizes and verify grid columns adjust)',
  );
});

Then('the grid should adjust columns:', async function (this: CustomWorld, _dataTable) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify grid columns adjust based on screen size from dataTable rows (verify each breakpoint has correct column count)',
  );
});
