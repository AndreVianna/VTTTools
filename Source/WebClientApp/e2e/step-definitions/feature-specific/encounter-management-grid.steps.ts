...
Given('I provide encounter with stage width {int} and height {int}', async function (
    this: CustomWorld,
    width: number,
    height: number
) {
...
Given('I have a encounter with stage, grid, and {int} placed assets', async function (
    this: CustomWorld,
    assetCount: number
) {
...
Then('I should see the encounter in my library', async function (this: CustomWorld) {
    const encounterId = (this as any).currentEncounterId;
    const encounters = await this.db.queryTable('Encounters', { Id: encounterId, OwnerId: this.currentUser.id });
...
Then('a new encounter should be created', async function (this: CustomWorld) {
    ...
    const encounters = await this.db.queryTable('Encounters', { Id: clonedEncounterId });
    expect(encounters.length).toBe(1);
});
...
Then('the stage configuration should be duplicated', async function (this: CustomWorld) {
    ...
    const sourceEncounters = await this.db.queryTable('Encounters', { Id: sourceEncounterId });
    const clonedEncounters = await this.db.queryTable('Encounters', { Id: clonedEncounterId });
...
Then('all {int} assets should be duplicated with new IDs', async function (this: CustomWorld, count: number) {
    ...
    const sourceEncounterId = (this as any).sourceEncounterId;
    const sourcePlacements = await this.db.queryTable('EncounterAssets', { EncounterId: sourceEncounterId });
    const clonedPlacements = await this.db.queryTable('EncounterAssets', { EncounterId: clonedEncounterId });
...
Then('each should have the correct grid configuration', async function (this: CustomWorld) {
    ...
    for (let i = 0; i < encounterIds.length; i++) {
        const encounters = await this.db.queryTable('Encounters', { Id: encounterIds[i] });
