...
Then(
  'Token preview should show {string} and Display preview should show {string}',
  async function (this: CustomWorld, _tokenImage: string, _displayImage: string) {
...
Then('the click should not trigger role assignment', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify clicking X button on image removes it without triggering role assignment logic (verify image was removed and no data-role change occurred)',
  );
});

Then('no keyboard modifiers should affect the removal', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify removing image works even if modifier keys are held (verify image removed regardless of Ctrl/Alt/Shift state)',
  );
});
...
Then(
  'the Display preview should show {string} \\(first Display)',
...
Then('should not show img3, img4, or img5 in previews', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify preview boxes only show first image of each role type (verify images 3-5 do not appear in either preview)',
  );
});
...
Then('this should be valid \\(no constraint against multiple Token images)', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify system allows multiple images with the same role without validation error (verify all Token images are accepted)',
  );
});
...
Then('I should be able to assign roles to any of them', async function (this: CustomWorld) {
  throw new Error(
    'OBSOLETE: Role assignment via keyboard shortcuts no longer exists. Tokens/portraits are uploaded via dedicated UI sections.',
  );
});
...
Then('entityId prop should be undefined \\(not editing)', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify component has undefined entityId in create mode (verify by checking component props or network requests have no entityId)',
  );
});
...
Then('entityId prop should be {string}', async function (this: CustomWorld, _assetId: string) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify component has correct entityId in edit mode (verify by checking component props or network requests include entityId)',
  );
});
...
Given(
  'the parent passes size prop: width={int}, height={int}',
...
Then(
  'the Token preview should render with {int}×{int} grid overlay',
...
Then('the grid should match the asset size', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify grid overlay dimensions match asset size (verify grid cell count matches width/height from size prop)',
  );
});
...
Then('onResourcesChange should be called with updated roles', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify parent onResourcesChange callback is invoked when image role changes (verify callback fired and includes updated role in resource array)',
  );
});
...
Then('onResourcesChange should be called with filtered array', async function (this: CustomWorld) {
  throw new Error(
    'NOT IMPLEMENTED: Step needs to verify parent onResourcesChange callback is invoked when image is removed (verify callback fired and removed image is not in resources array)',
  );
});
...
Then('the upload mutation should be called with entityId=undefined', async function (this: CustomWorld) {
  ...
  const response = await this.page.waitForResponse(
    (resp) => resp.url().includes('/api/resources') && resp.request().method() === 'POST',
  );
...
Then(
  'the upload mutation should be called with entityId={string}',
  async function (this: CustomWorld, assetId: string) {
    const response = await this.page.waitForResponse(
      (resp) => resp.url().includes('/api/resources') && resp.request().method() === 'POST',
    );
    const requestBody = response.request().postDataJSON();
