...
export async function navigateToRegistrationPage(world: CustomWorld): Promise<void> {
  ...
  const registerLink = world.page.getByRole('link', {
 ...
 */
export async function fillRegistrationForm(page: Page, email: string, name: string, password: string): Promise<void> {
  await page.getByLabel(/email/i).fill(email);
...
export async function submitRegistrationForm(world: CustomWorld): Promise<void> {
  ...
  const passwordInput = world.page.getByRole('textbox', {
    name: /^password$/i,
  });
 ...
 */
export async function performLogin(world: CustomWorld, email: string, password: string): Promise<void> {
 ...
 */
export async function switchAuthMode(page: Page, targetMode: 'login' | 'register' | 'reset-password'): Promise<void> {
  switch (targetMode) {
    case 'register':
 ...
 */
export async function verifyFieldError(page: Page, fieldLabel: string, errorMessage: string): Promise<void> {
 ...
 */
export async function simulateNetworkFailure(page: Page, endpoint: 'login' | 'register' | 'logout'): Promise<void> {
 ...
 */
export async function simulateServerError(page: Page, endpoint: 'login' | 'register' | 'logout'): Promise<void> {
...
export async function performPoolUserLogin(world: CustomWorld): Promise<void> {
  ...
  await world.page.waitForURL((url) => !url.pathname.includes('/login') && !url.pathname.includes('/register'), {
    timeout: 15000,
  });
  await world.page.waitForLoadState('networkidle');
}
