...
export async function setCellWidth(page: Page, width: number): Promise<void> {
  const input = page
    ...
    .filter({ hasText: 'Cell Width' })
    .or(page.locator('label:has-text("Cell Width")').locator('..').locator('input'));
...
export async function setCellHeight(page: Page, height: number): Promise<void> {
  const input = page
    .locator('input[type="number"]')
    .filter({ hasText: 'Cell Height' })
    .or(page.locator('label:has-text("Cell Height")').locator('..').locator('input'));
...
export async function toggleSnapToGrid(page: Page, enabled: boolean): Promise<void> {
  const checkbox = page
    ...
    .filter({ hasText: 'Snap assets to grid' })
    .or(page.locator('label:has-text("Snap assets to grid")').locator('..').locator('input[type="checkbox"]'));
...
export async function saveGridConfiguration(page: Page): Promise<void> {
  ...
  await page.waitForResponse(
    (response) => response.url().includes('/api/library/encounters') && response.request().method() === 'PATCH',
    { timeout: 5000 },
  );
}
...
export async function openGridConfigPanel(page: Page): Promise<void> {
  ...
  const gridButton = page.locator('button[aria-label="Configure Grid"]').or(page.locator('button:has-text("Grid")'));
...
export async function verifyValidationError(page: Page, errorMessage: string): Promise<void> {
  const alert = page.locator('[role="alert"]', { hasText: errorMessage }).or(page.locator(`text=${errorMessage}`));
