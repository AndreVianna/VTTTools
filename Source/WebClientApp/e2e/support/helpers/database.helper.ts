...
export class DatabaseHelper {
  ...
  async verifyAssetExists(assetId: string, expectedFields: Partial<any>): Promise<void> {
    ...
    for (const [key, expectedValue] of Object.entries(expectedFields)) {
      if (asset[key] !== expectedValue) {
        throw new Error(`Asset ${key} mismatch: expected ${expectedValue}, got ${asset[key]}`);
      }
    }
  }
  ...
  async verifyAssetTokenCount(assetId: string, expectedCount: number): Promise<void> {
    ...
    if (tokens.length !== expectedCount) {
      throw new Error(`Asset token count mismatch: expected ${expectedCount}, got ${tokens.length}`);
    }
  }
   ...
   */
  async insertPasswordResetAttempt(attempt: { email: string; attemptedAt: Date }): Promise<void> {
    const query = `
   ...
   * Insert user session
   */
  async insertUserSession(session: { userId: string; deviceInfo: string; createdAt: Date }): Promise<void> {
    const query = `
  ...
  async verifyEncounterStage(encounterId: string, expectedStage: Partial<any>): Promise<void> {
    ...
    for (const [key, expectedValue] of Object.entries(expectedStage)) {
      if (stageConfig[key] !== expectedValue) {
        throw new Error(`Stage ${key} mismatch: expected ${expectedValue}, got ${stageConfig[key]}`);
      }
    }
  }
  ...
  async verifyEncounterGrid(encounterId: string, expectedGrid: Partial<any>): Promise<void> {
    ...
    for (const [key, expectedValue] of Object.entries(expectedGrid)) {
      if (gridConfig[key] !== expectedValue) {
        throw new Error(`Grid ${key} mismatch: expected ${expectedValue}, got ${gridConfig[key]}`);
      }
    }
  }
  ...
  async verifyEncounterAssetPlacementCount(encounterId: string, expectedCount: number): Promise<void> {
    const placements = await this.queryTable('EncounterAssets', {
      EncounterId: encounterId,
    });

    if (placements.length !== expectedCount) {
      throw new Error(`Encounter asset placement count mismatch: expected ${expectedCount}, got ${placements.length}`);
    }
  }
   ...
   */
  async insertAdventure(adventure: { name: string; ownerId: string; type: string }): Promise<string> {
    ...
    const params = [adventureId, adventure.name, adventure.ownerId, adventure.type, '', new Date(), new Date()];
  ...
  async insertGameSession(session: {
    ...
    const players = JSON.stringify([{ userId: session.ownerId, role: 'Master' }]);
  ...
  async verifyGameSessionExists(sessionId: string, expectedFields: Partial<any>): Promise<void> {
    ...
    for (const [key, expectedValue] of Object.entries(expectedFields)) {
      if (session[key] !== expectedValue) {
        throw new Error(`GameSession ${key} mismatch: expected ${expectedValue}, got ${session[key]}`);
      }
    }
  }
   ...
   */
  async verifyGameSessionParticipant(sessionId: string, userId: string, expectedRole: string): Promise<void> {
    const [session] = await this.queryTable('GameSessions', { Id: sessionId });
    ...
    if (participant.role !== expectedRole) {
      throw new Error(`Participant role mismatch: expected ${expectedRole}, got ${participant.role}`);
    }
  }
