...
window.initStage = function (canvasContainer, stageData) {
    ...
    window.stage.size = { x: stageData.size.width + (2 * window.stage.padding), y: stageData.size.height + (2 * window.stage.padding) };
...
window.getCachedImage = function(src) {
...
//    console.log("Drawing background.");
    ...
    //} else {
        ...
        img.onerror = function () {
            ...
            window.stage.backgroundUrl = null;
        };
    //}
    img.src = imageUrl;
    //ctx.restore();
...
function drawSingleAsset(ctx, asset) {
    ...
    if (asset.imageSrc) {
        const img = getCachedImage(asset.imageSrc);
        if (img.complete) {
            drawAssetImage(ctx, img, asset, x, y, sizeX, sizeY);
        } else {
            img.onload = function () { drawAssetImage(ctx, img, asset, x, y, sizeX, sizeY); };
            img.onerror = function() {
