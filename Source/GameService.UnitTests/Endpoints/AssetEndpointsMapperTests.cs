...
public class AssetEndpointsMapperTests {
    ...
    public void MapAssetManagementEndpoints_RegistersEndpoints() {
        ...
        groupDataSource.Endpoints[3].DisplayName.Should().Be("HTTP: PATCH /api/assets/{id:guid} => UpdateAssetHandler");
        groupDataSource.Endpoints[4].DisplayName.Should().Be("HTTP: DELETE /api/assets/{id:guid} => DeleteAssetHandler");
        groupDataSource.Endpoints[5].DisplayName.Should().Be("HTTP: POST /api/assets/{id:guid}/upload => UploadAssetFileHandler");
    }
}