...
public class AdventureEndpointsMapperTests {
    ...
    public void MapAdventureManagementEndpoints_RegistersEndpoints() {
        ...
        groupDataSource.Endpoints[2].DisplayName.Should().Be("HTTP: GET /api/adventures/{id:guid}/episodes => GetEpisodesHandler");
        groupDataSource.Endpoints[3].DisplayName.Should().Be("HTTP: POST /api/adventures/{id:guid}/episodes => CreateEpisodeHandler");
        groupDataSource.Endpoints[4].DisplayName.Should().Be("HTTP: POST /api/adventures/ => CreateAdventureHandler");
        groupDataSource.Endpoints[5].DisplayName.Should().Be("HTTP: PATCH /api/adventures/{id:guid} => UpdateAdventureHandler");
        groupDataSource.Endpoints[6].DisplayName.Should().Be("HTTP: DELETE /api/adventures/{id:guid} => DeleteAdventureHandler");
        groupDataSource.Endpoints[7].DisplayName.Should().Be("HTTP: POST /api/adventures/{id:guid}/clone => CloneAdventureHandler");
    }
}