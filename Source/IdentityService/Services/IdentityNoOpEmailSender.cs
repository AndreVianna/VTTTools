...
internal sealed class IdentityNoOpEmailSender : IEmailSender<NamedUser> {
    ...
    public Task SendConfirmationLinkAsync(NamedUser user, string email, string confirmationLink)
    ...
    public Task SendPasswordResetLinkAsync(NamedUser user, string email, string resetLink)
        => _emailSender.SendEmailAsync(email, "Reset your password", $"Please reset your password by <a href='{resetLink}'>clicking here</a>.");

    public Task SendPasswordResetCodeAsync(NamedUser user, string email, string resetCode)
