...
import type { Campaign, Adventure } from '../../../types/domain';
...
export interface CampaignDetailPageProps {
  ...
  mediaBaseUrl: string;
  onBack: () => void;
  onUpdateCampaign: (request: { name?: string; description?: string; isPublished?: boolean; backgroundId?: string }) => Promise<void>;
  onUploadFile: (file: File, type: string, resource: string, entityId: string) => Promise<{ id: string }>;
  onCreateAdventure: (request: { name: string; description: string; style: number }) => Promise<{ id: string }>;
      ...
      <Box sx={{ p: 3 }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
            <IconButton id='btn-back-to-library' onClick={onBack} aria-label='Back to library'>
              <ArrowBackIcon />
            </IconButton>
            <Box sx={{ flex: 1 }}>
              <TextField
