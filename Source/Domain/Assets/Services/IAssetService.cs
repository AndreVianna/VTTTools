...
public interface IAssetService {
    ...
    Task<Asset[]> GetAssetsAsync(Guid userId, AssetKind? kind, string? search, bool? published, string? owner, CancellationToken ct = default);
    Task<(Asset[] assets, int totalCount)> GetAssetsPagedAsync(Guid userId, AssetKind? kind, string? search, bool? published, string? owner, int skip, int take, CancellationToken ct = default);
