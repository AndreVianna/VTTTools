...
public sealed partial class HierarchicalFileStore(string rootPath)
    ...
    public IReadOnlyList<string> GetExistingImageFiles(
        ...
        return !Directory.Exists(variantPath)
            ? []
            : [.. ImageTypeFor(asset.Classification.Kind).Select(it => Path.Combine(variantPath, BuildFileName(it, variantIndex) + _imageFileExtension)).Where(File.Exists)];
    }
    ...
    public IReadOnlyList<string> GetExistingPromptFiles(Asset asset, int variantIndex = 0) {
        ...
        return !Directory.Exists(variantPath)
            ? []
            : [.. ImageTypeFor(asset.Classification.Kind).Select(it => Path.Combine(variantPath, BuildFileName(it, variantIndex) + _promptFileExtension)).Where(File.Exists)];
    }
