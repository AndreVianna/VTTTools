...
public sealed partial class FileTokenStore(string root) : IFileTokenStore {
    ...
    public string GetEntityFolder(string idOrSlug)
        => string.IsNullOrWhiteSpace(idOrSlug) ? throw new ArgumentException("Entity ID cannot be empty.", nameof(idOrSlug))
            : !ValidEntityId().IsMatch(idOrSlug) ? throw new ArgumentException($"Invalid entity ID format: {idOrSlug}", nameof(idOrSlug))
            : Directory.GetDirectories(_root, $"*-{idOrSlug}", SearchOption.TopDirectoryOnly).FirstOrDefault() ?? Path.Combine(_root, idOrSlug);
    ...
    public IEnumerable<(string Folder, TokenMetadata Metadata)> EnumerateTokens() {
        ...
        foreach (var dir in Directory.GetDirectories(_root)) {
            var metaPath = Path.Combine(dir, "token_1.json");
            if (!File.Exists(metaPath))
                continue;

            var json = File.ReadAllTextAsync(metaPath).GetAwaiter().GetResult();
