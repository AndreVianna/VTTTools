...
- When you have a plan approved to execution or have a complex task to execute, you will create a TODO list with small atomic steps, you will delegate each step to the proper agent to execute and after each step you will use the reviewer agent to analyze, validate, and grade the work of the previous step and see if there any corrections that need to be made before proceeding. you will get the response from the reviewer and decide if you need to resubmit the corrections again to the proper agent to fix. You will continue that throughout every step without interruption, even if the context is low. At the end you will ask the reviewer to analyze and grade the whole process and generate a final report that you will present to me here (no document needed). After each step update the memory mcp with the current state of the task in case the context needs to be compacted. If you already have a todo list related to the same task you will update the todo list accordingly. If you have a todo list for a different task you will ask the user if you should abandon the previous task first. If yes you will clear the previous todo list.